...
<div class="grouptop">
	...
	<?php
	if (!empty($_['userExists'])) {
		?>
		<p class="warning">
			<?php p($l->t('A user with that name already exists.')); ?>
		</p>
		<?php
	} 
	?>
...
<div class="submit-wrap">
	...
	if (!empty($_['unable'])) {
		?>
		<p class="warning">
			<?php p($l->t('Unable create user.')); ?>
		</p>
		<?php
	}elseif(!empty($_['created'])){ 
		?>
		<p class="warning">
			<?php p($l->t('User was created.')); ?>
	    </p>
	<?php
	}elseif(!empty($_['invalidpassword'])){
		?>
		<p class="warning">
			<?php p($l->t('Password of least length 8.')); ?>
		</p>
		<p class="warning">
			<?php p($l->t('Password contanin at least one uppercase letter.')); ?>
		</p>
		<p class="warning">
			<?php p($l->t('Password contanin at least one number.')); ?>
		</p>
	<?php 
	}
	?>
